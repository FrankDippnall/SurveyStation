...
// If the submitted username and password is found in the table, then the following session variable is set: $_SESSION["loggedIn"]=true;
...
if (isset($_COOKIE["r_username"])) {
	$username = $_COOKIE["r_username"];
	$auto_rememberme = true;
}
else {
	$username = "";
	$auto_rememberme = false;
}
...
elseif (isset($_POST['username']))
{
	...
	if ($errors == "")
	{
		...
		$sql =  "SELECT email FROM users WHERE username = '$username' AND hash = '$passhash'";
		...
		if ($n > 0) {
			...
			//setup remember me 
			if ($rememberme) setcookie('r_username', $username,time()+60*60*24*30);
			else {setcookie('r_username', "",time()-60);} //if not remember me, then forget me
			//attempt js redirect
			echo "<script> window.location.assign('manage_account.php?login=true'); </script>";
...
if ($show_signin_form)
{
	...
	$remembercheck = "";
	if ($auto_rememberme) $remembercheck = " checked"; 
// show the form that allows users to log in
// Note we use an HTTP POST request to avoid their password appearing in the URL:
echo <<<_END
<form action="sign_in.php" method="post" class="noerrors">
  Please enter your username and password:<br>
  <div class="fields">
  <p>
  <label>Username </label><input type="text" name="username" maxlength="16" value="$username" required> <span class="error"></span>
  <br>
  </p>
  <p>
  <label>Password </label><input type="password" name="password" maxlength="16" value="$password" required> <span class="error"></span>
  </p>
  
  </div>
  <div class="rememberme"><label>Remember Me</label><input type="checkbox" name="rememberme"$remembercheck> </div>
  <br>
